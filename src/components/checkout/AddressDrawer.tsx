...
export function AddressDrawer({
  ...
  const validateForm = () => {
    ...
    if (formData.addressLine.length > 255) {
      return "Address exceeds 255 characters.";
    }
  ...
  return (
    <>
      ...
      {/* Right Drawer */}
      <div
        className={`fixed inset-y-0 right-0 z-50 w-full max-w-[420px] bg-white shadow-2xl transition-transform duration-300 ease-in-out transform flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        {/* Header */}
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`text-xs font-medium ${formData.addressLine.length >= 255 ? "text-red-500" : "text-gray-400"}`}
                  >
                ...
                <div className="flex gap-3">
                  {["HOME", "WORK", "OTHER"].map((type) => (
                    <button
                      key={type}
                      onClick={() =>
                        setFormData({ ...formData, label: type as any })
                      }
